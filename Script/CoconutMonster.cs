...
public class CoconutMonster : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        playerHp = playerDetials.GetHealth();
        if (!isAttack ) { transform.Rotate(new Vector3(0, 0, 0), Space.Self); } // หมุนกลับเมื่อไม่ได้โจมตี
        if (Vector3.Distance(transform.position, player.position) >= 12)
        {
            rigidbody2D.linearVelocity = new Vector2(0, 0);
        }

        if (Vector3.Distance(transform.position, player.position) < 12 && !isAttack && playerHp > 0) { Movement(); }

        if (attacking && Time.time > skillRate && playerHp > 0)
        {
            Attack();
    ...
    private void Shooted() // ยิง shootObject เรียกใช้จาก animation
    {
        if (direction > 1.75)
        {
            Instantiate(shootObject, new Vector3(transform.position.x - 1.2f, transform.position.y + 1.15f, transform.position.z), Quaternion.Euler(0, 180, -7.5f));
        }
        else if (direction < -1.75)
        {
            Instantiate(shootObject, new Vector3(transform.position.x + 1.2f, transform.position.y + 1.15f, transform.position.z), Quaternion.Euler(0, 0, -7.5f));
        }
    }
